 ...
 */
public class Cuboid implements Iterable<Block>, Cloneable, Serializable {
    ...
    public Cuboid raise(int amount) {
        this.y1 += amount;
        this.y2 += amount;
     ...
     */
    public List<Block> getBlocksThatArent(ItemData... itemData) {
        Iterator<Block> blockI = this.iterator();
        List<Block> copy = new ArrayList<>();
        boolean brk;
        while (blockI.hasNext()) {
            brk = false;
            Block blocc = blockI.next();

            for(ItemData data : itemData) {
                if(data.getMaterial() == blocc.getType() && ((byte) data.getData()) == blocc.getData()) {
                    brk = true;
                    break;
                }
            }

            if(!brk) copy.add(blocc);
        }
        return copy;
    }
     ...
     * @return The blocks in the Cuboid
     */
    public List<Block> getBlocks(Material mat) {
        Iterator<Block> blockI = this.iterator();
        List<Block> copy = new ArrayList<>();
        while (blockI.hasNext()) {
            Block blocc = blockI.next();

            if(blocc.getType() == mat) copy.add(blocc);
        }
        return copy;
    }
     ...
     * @return The blocks in the Cuboid
     */
    public List<Block> getBlocks(ItemData data) {
        Iterator<Block> blockI = this.iterator();
        List<Block> copy = new ArrayList<>();
        while (blockI.hasNext()) {
            Block blocc = blockI.next();

            if(blocc.getType() == data.getMaterial() && blocc.getData() == data.getData()) copy.add(blocc);
        }
        return copy;
    }
     ...
     * @return The blocks in the Cuboid
     */
    public List<Block> getBlocks(int yChange) {
        Iterator<Block> blockI = this.iterator(yChange);
        List<Block> copy = new ArrayList<Block>();
        while (blockI.hasNext())
            copy.add(blockI.next());
        return copy;
    ...
    public List<Block> getSolidBlocks() {
        ...
        while (blockI.hasNext()) {
            Block b = blockI.next();
            if(b.getType() != Material.AIR) copy.add(b);
        }
        return copy;
    }

    public List<Block> getAllBlocksOpposite(Material mat) {
        Iterator<Block> blockI = this.iterator();
        List<Block> copy = new ArrayList<Block>();
        while (blockI.hasNext()) {
            Block b = blockI.next();
            if(b.getType() != mat) copy.add(b);
        }
        return copy;
    }
    ...
    public Iterator<Block> iterator(int yChange) {
        return new CuboidIterator(this.getWorld(), this.x1, this.y1 + yChange, this.z1, this.x2, this.y2 + yChange, this.z2);
    }
    ...
    public String serialize() {
        return new FakeLocation(getLowerNE()).serialize() + "::" + new FakeLocation(getUpperSW()).serialize();
    }
    ...
    public void deserialize(String str) {
        ...
        FakeLocation a = new FakeLocation(split[0]);
        FakeLocation b = new FakeLocation(split[1]);
        Location l1 = a.toLocation(), l2 = b.toLocation();
