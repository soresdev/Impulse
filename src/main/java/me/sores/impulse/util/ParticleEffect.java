...
import me.sores.impulse.util.reflect.ReflectionUtils;
...
import java.util.Map;
    ...
    public enum ParticleEffect {
         ...
         * @see ParticlePacket#sendTo(Location, double)
         */
        public void display(ParticleColor color, float xOffset, float yOffset, float zOffset, Location center, double range) throws ParticleVersionException, ParticleColorException {
            if (!isSupported()) {
                throw new ParticleVersionException("This particle effect is not supported by your server version");
            }
            if (!hasProperty(ParticleProperty.COLORABLE)) {
            ...
            new ParticlePacket(this, xOffset, yOffset, zOffset, color, range > 256).sendTo(center, range);
        }
         ...
         * @see ParticlePacket#sendTo(Location, double)
         */
        public void display(ParticleColor color, float xOffset, float yOffset, float zOffset, Location center, List<Player> players) throws ParticleVersionException, ParticleColorException {
            if (!isSupported()) {
                throw new ParticleVersionException("This particle effect is not supported by your server version");
            }
            if (!hasProperty(ParticleProperty.COLORABLE)) {
            ...
            new ParticlePacket(this, xOffset, yOffset, zOffset, color, isLongDistance(center, players)).sendTo(center, players);
        }
        ...
        public static final class ParticlePacket {
            ...
            public ParticlePacket(ParticleEffect effect, float offsetX, float offsetY, float offsetZ, ParticleColor color, boolean longDistance) {
                this(effect, offsetX, offsetY, offsetZ, 1, 0, longDistance, null);
//                if (effect == ParticleEffect.REDSTONE && color instanceof OrdinaryColor && ((OrdinaryColor) color).getRed() == 0) {
//                    offsetX = Float.MIN_NORMAL;
//                }
                this.colorR = color.getValueX();
                this.colorG = color.getValueY();
                this.colorB = color.getValueZ();
            }

            private float colorR = 0, colorG = 0, colorB = 0;
            ...
            private void initializePacket(Location center) throws PacketInstantiationException {
                ...
                try {
                    ...
                    ReflectionUtils.setValue(packet, true, "b", (float) center.getX() + offsetX);
                    ReflectionUtils.setValue(packet, true, "c", (float) center.getY() + offsetY);
                    ReflectionUtils.setValue(packet, true, "d", (float) center.getZ() + offsetZ);
                    ReflectionUtils.setValue(packet, true, "e", colorR);
                    ReflectionUtils.setValue(packet, true, "f", colorG);
                    ReflectionUtils.setValue(packet, true, "g", colorB);
